...
Do something.
""")
            agent = load_agent(tmpdir, "test-agent")
            assert agent.run_agent == "codex"
            assert "# Test Agent" in agent.system_context
            assert "This is a test agent." in agent.description
            assert agent.file_path.endswith("test-agent.md")
